...
import { createDeepSeekPlan } from "../src/core/deepseek-planner.js";
...
const key = process.env.DEEPSEEK_API_KEY || (await readKeyFromStdin());
if (!key) {
...
const settings = {
  ...
  deepseekApiKey: key,
  deepseekModel: process.env.DEEPSEEK_MODEL || DEFAULT_SETTINGS.deepseekModel,
  ...
  windows: [
    {
      ...
      focused: true,
      tabs: [
        { id: 10, title: "DeepSeek JSON Output docs", url: "https://api-docs.deepseek.com/guides/json_mode", active: true },
        { id: 11, title: "Chrome tabGroups API", url: "https://developer.chrome.com/docs/extensions/reference/api/tabGroups" },
        { id: 12, title: "OpenAI Structured Outputs", url: "https://developers.openai.com/api/docs/guides/structured-outputs" },
...
const plan = await createDeepSeekPlan(inventory, settings);
...
async function readKeyFromStdin() {
  if (process.stdin.isTTY) {
    output.write("Provide DEEPSEEK_API_KEY as an environment variable or pipe it on stdin.\n");
    return "";
  }
