...
Chrome's "current window" can differ from the focused/topmost window, especially in service workers. Current-window mode should resolve the active scope from the side panel or action invocation and pass an explicit `windowId` through the job. Do not depend on `WINDOW_ID_CURRENT` inside background code.
